...
import { ScrollReveal } from '@/components/effects/InteractionEffects';
...
const EmergencyServices = () => {
    ...
    return (
        <div className="min-h-screen pb-10 relative overflow-hidden">
            <div className="max-w-6xl mx-auto px-4 lg:px-6 relative z-10 pt-6 bg-background/40 backdrop-blur-2xl rounded-[2rem] border border-border/10 shadow-2xl my-6 pb-10 space-y-5 animate-in fade-in duration-700">

                <div className="flex flex-col md:flex-row md:items-start justify-between gap-4">
                    <div className="space-y-1">
                        <h1 className="text-3xl xl:text-4xl font-black tracking-tight flex items-center gap-2.5">
                            <AlertTriangle className="text-rose-500 w-8 h-8" />
                            Crisis Command Center
                        </h1>
                        <p className="text-sm text-muted-foreground">Unified Incident Response Interface • Almaty V5.0</p>
                    </div>

                    {status && (
                        <div className="flex items-center gap-4 bg-white/50 dark:bg-card/40 backdrop-blur-md p-3 rounded-xl border border-border/50 shadow-xl card-3d">
                            <div className="flex flex-col items-end">
                                <span className="text-[10px] font-black uppercase tracking-widest text-muted-foreground">City Safety Pulse</span>
                                <span className={`text-2xl font-black ${status.safety_score > 80 ? 'text-emerald-500' : 'text-rose-500'}`}>
                                    {status.safety_score}%
                                </span>
                            </div>
                            <div className={`w-12 h-12 rounded-full border-4 flex items-center justify-center ${status.safety_score > 80 ? 'border-emerald-500/30 text-emerald-500' : 'border-rose-500/30 text-rose-500'}`}>
                                <Activity size={24} />
                            </div>
                        </div>
                    )}
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
                    ...
                    <div className="space-y-5">
                        <Card className="border-rose-400/35 bg-rose-500/5 relative overflow-hidden group card-3d shadow-md">
                            ...
                            <CardHeader className="relative z-10 pb-3">
                                ...
                                <CardDescription className="text-xs">Broadcast your location to all services</CardDescription>
                            </CardHeader>
                            <CardContent className="space-y-4 relative z-10">
                                <button
                                    onClick={handleSOS}
                                    disabled={sosMutation.isPending}
                                    className="w-full h-44 rounded-[999px] bg-rose-500 hover:bg-rose-600 active:scale-95 transition-colors shadow-[0_0_18px_rgba(244,63,94,0.24)] flex flex-col items-center justify-center text-white border-8 border-rose-300/25"
                                >
                                    {sosMutation.isPending ? (
                                        <Loader2 className="w-12 h-12 animate-spin" />
                                    ) : (
                                        <>
                                            <PhoneCall className="w-12 h-12 mb-1.5" />
                                            <span className="text-3xl font-black tracking-tighter uppercase">SOS</span>
                                        </>
                                    )}
                                </button>
                                <p className="text-[10px] text-center text-rose-700/70 dark:text-rose-300/70 font-medium uppercase tracking-widest">
                        ...
                        <Card className="border-border/50 card-3d bg-white/5 dark:bg-card/40 backdrop-blur-md overflow-hidden shadow-xl">
                            ...
                            <CardContent className="p-3">
                                <div className="grid grid-cols-2 gap-2">
                                    ...
                                    <a
                                        href="tel:112"
                                        className="col-span-2 flex items-center justify-between p-3 rounded-xl bg-rose-500 hover:bg-rose-600 transition-colors group relative overflow-hidden shadow-md shadow-rose-500/20"
                                    >
                                        <div className="flex items-center gap-3 relative z-10">
                                            ...
                                            <div>
                                                ...
                                                <div className="text-2xl font-black text-white leading-none">112</div>
                                            </div>
                                        </div>
                                    ...
                                    {[
                                        ...
                                        >
                                            <contact.icon className={cn("w-5 h-5 mb-1.5 transition-transform group-hover:scale-110", contact.color)} />
                                            <span className="text-lg font-black tracking-tighter">{contact.number}</span>
                                    ...
                                    <div className="space-y-3">
                                        <div className="grid grid-cols-3 gap-2 mb-4">
                                            ...
                                            <div className="text-center p-2 rounded-lg bg-rose-500/10 border border-rose-500/20">
                                                <div className="text-xl font-black text-rose-500">{units.filter((u: any) => u.type === 'AMBULANCE').length}</div>
                                        ...
                                        <div className="max-h-[210px] overflow-y-auto pr-2 space-y-2">
                                            {units.slice(0, 6).map((unit: any) => (
                                                <div key={unit.id} className="flex items-center justify-between p-2 rounded-lg bg-muted/30 border border-border/10 text-xs">
                                                    <div className="flex items-center gap-2">
                                                        <div className={`w-2 h-2 rounded-full ${unit.status === 'ON_SCENE' ? 'bg-rose-500' : 'bg-emerald-500'}`} />
                                ...
                                {isLoading ? (
                                    ...
                                    <div className="divide-y divide-border/10">
                                        {incidents.map((incident: any) => (
                                            <div key={incident.id} className="p-4 flex flex-col md:flex-row md:items-center justify-between hover:bg-muted/30 transition-all group">
                                                <div className="flex gap-4">
                                                    <div className={`mt-1 p-2.5 rounded-xl ${incident.type === 'FIRE' ? 'bg-orange-500/10 text-orange-500' :
                                                        incident.type === 'ACCIDENT' ? 'bg-rose-500/10 text-rose-500' :
                                                        ...
                                                        {incident.type === 'FIRE' ? <Flame size={20} /> :
                                                            incident.type === 'ACCIDENT' ? <AlertTriangle size={20} /> :
                                                                incident.type === 'POLICE' ? <Shield size={20} /> :
                                                                    <Stethoscope size={20} />}
                                                    </div>
                                                    <div className="space-y-1">
                                                        <div className="flex items-center gap-3">
                                                            <h4 className="font-bold text-base">{incident.type}</h4>
                                                            <Badge className={
                                                                incident.severity === 'CRITICAL' ? 'bg-rose-600' :
                                                        ...
                                                        <div className="flex items-center gap-3 text-[11px] font-mono text-muted-foreground mt-2">
                                                            <span className="flex items-center gap-1">
                                                                <MapPin className="w-3 h-3 text-rose-500" /> {incident.lat.toFixed(4)}, {incident.lng.toFixed(4)}
                                                            </span>
                                                            ...
                                                            {incident.assigned_units?.length > 0 && (
                                                                <span className="flex items-center gap-1.5 text-blue-500 font-bold">
                                                                    <Activity size={12} /> DISPATCHED: {incident.assigned_units.join(', ')}
                                                                </span>
                                                            )}
                                                        </div>
                                                    </div>
                                    ...
                                    <div className="p-8 text-center">
                                        <Shield className="w-12 h-12 text-muted-foreground mx-auto mb-3 opacity-20" />
                                        <h3 className="text-lg font-bold mb-1 text-muted-foreground">All Clear</h3>
